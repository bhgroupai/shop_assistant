...
- **MVP finished 2026-09-21** (tickets 1–15 Done; Sanjarbek finished #4–#15 with parallel agents after the juniors stalled). Eval: 19/20, 0 invented, 4 semantic-only, median 4.7 s. Bot live on Codeschool as `shop-assistant.service` (reuses telegram_digest's bot token — that service is stopped/disabled), data in `~/shop_assistant/data/`.
- **S4 done 2026-09-21** (#20, #21, #19 — Sanjarbek, two-agent flow, admin-merged PRs #19–#25). Outcomes: product names = type + brand (`product_name`), announcements → `boshqa` (`is_announcement`) and **search now excludes `boshqa`** (`search.is_sellable`, D-9 — it never did before); replies numbered + `narxi: so'rab beraman` + offer line; **customer reply is a one-message carousel** (photo + card caption + ◀ N/n ▶ / Narxini so'rash / Kanalda ko'rish, D-8) — forwards and albums were rejected live (too much scrolling; albums can't carry buttons; Telegram's native "Show as carousel" is app-only, not in Bot API 10.x / layer 229). Server data re-extracted (backups `data/products.jsonl.bak*-2026-09-21`). Last recorded eval 18/20 (before PR #24 fixed both misses); **final eval still owed** — Ollama on Codeschool hung on 2026-09-22 ~00:00 (`curl /api/generate` times out) and needs a restart first. #16–#18 stay in R2 backlog.
- **Deploying from this machine:** `deploy.sh`'s remote `uv` fails (not on non-interactive PATH → use `export PATH=$HOME/.local/bin:$PATH`); auto-mode blocks deploys/merges — the user runs them via `! …` or says "ask again".
