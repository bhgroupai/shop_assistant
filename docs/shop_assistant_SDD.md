...
| 1.0 | 2026-09-23 | SRS v0.5 FR-13a (#24): new `lang.py` (§3.8a: language detection, per-chat store, `TEXTS`), `data/languages.json` (§2.7), `config.LANGUAGES_PATH`; §3.7 explicit "Answer in …" line + `run_agent(lang=)`; §3.8 `/start` greeting without LLM, localized carousel/escalation/error texts |
