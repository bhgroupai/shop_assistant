...
- Manual function-calling loop over `llm.client().models.generate_content` (automatic function calling **off**): each model `function_call` runs the matching `TOOLS` entry, the model turn and the `function_response` are appended, repeat until the model answers with text or `config.MAX_ITERATIONS = 8` requests were made (then the polite apology). Tool declarations come from `TOOLS` via `llm.tool_declarations` (single source of truth). History keeps the `{"role": "user"|"assistant", "content": str}` format; assistant turns are sent to Gemini as role `model`. Timeout `llm.AGENT_TIMEOUT_S` (8 s, client side) per request, the retry included; the request also sends `X-Server-Timeout: 10` (`llm.SERVER_DEADLINE_S`) because the Gemini API rejects a server deadline under 10 s, which the SDK would otherwise derive from the 8 s timeout.
...
| 1.5 | 2026-09-23 | #26: `llm.AGENT_TIMEOUT_S` 15 → 8 s (client side; explicit `X-Server-Timeout: 10` since the API minimum deadline is 10 s); §3.7 one retry per model step on 500/502/503/504 or timeout (same model, 0.7 s pause, one WARNING), no retry on 429/400; extraction/embedding retries unchanged |
