...
Version 1.6 · 2026-09-24 · Status: draft · Implements: shop_assistant_SRS.md v0.5
...
- Manual function-calling loop over `llm.client().models.generate_content` (automatic function calling **off**): each model `function_call` runs the matching `TOOLS` entry, the model turn and the `function_response` are appended, repeat until the model answers with text or `config.MAX_ITERATIONS = 8` requests were made (then the polite apology). Tool declarations come from `TOOLS` via `llm.tool_declarations` (single source of truth). History keeps the `{"role": "user"|"assistant", "content": str}` format; assistant turns are sent to Gemini as role `model`. Timeout `llm.AGENT_TIMEOUT_S` (15 s, client side) per request, the retry included; the request also sends `X-Server-Timeout` = `llm.SERVER_DEADLINE_S` (max(10, timeout)) because the Gemini API rejects a server deadline under 10 s. 8 s was tried on 2026-09-23 and failed every live turn while Google was overloaded.
...
- Errors (#26): each model step that fails with a server error 500/502/503/504 or a client timeout is retried **once**, same model and same contents (tool results are resent, tools are not re-run), after a `RETRY_PAUSE_S` (0.7 s) pause, with one WARNING. If the retry fails too, or the error is not transient (429 quota, 400 invalid key, missing key), one ERROR log line, the customer gets `APOLOGY` (or `TEXTS[lang]["error_reply"]`), the failed turn is not stored. `last_run["llm_calls"]` counts every attempt. No backup model and no paid fallback when the daily quota runs out (SRS NFR-2).
...
- One Telethon bot client. Handlers:
  ...
  - **No tool lines to customers (#27).** `handle_customer` passes the agent's reply through `strip_tool_lines` (whole lines matching `tools.TOOL_LINE_RE` removed, blank runs collapsed, everything else byte-for-byte) before `send_reply`, so neither the plain text, the carousel caption/buttons nor `_captions` carry the paging line; the prompt (§3.7 rule 10) also says never to copy it and to give the total in the customer's language. Owner relays are not filtered.
...
| 1.5 | 2026-09-23 | #26: `llm.AGENT_TIMEOUT_S` stays 15 s (8 s was tried and failed every live turn while Google was overloaded); explicit `X-Server-Timeout` = max(10, timeout) since the API minimum deadline is 10 s; §3.7 one retry per model step on 500/502/503/504 or timeout (same model, 0.7 s pause, one WARNING), no retry on 429/400; extraction/embedding retries unchanged |
| 1.6 | 2026-09-24 | #27: the listing tools' range/past-the-end line never reaches a customer — `tools.TOOL_LINE_RE` next to `_page`, `bot.strip_tool_lines` applied in `handle_customer` (plain + carousel, owner relay untouched); §3.7 prompt rule 10 "ko'rsatildi line is for you only, tell the total instead"; §3.6, §3.8 |
